...
<div class="view-container">
    <div class="card" style="background: var(--bg-card); border-radius: var(--radius-lg); box-shadow: var(--shadow-md); border: 1px solid #f1f5f9; padding: 24px; margin-bottom: 30px;">
        <div class="card-title-bar" style="border-bottom: 1px solid #f1f5f9; padding-bottom: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
            <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 18px; color: var(--text-primary); margin: 0;">📅 Follow-up Reminders</h2>
            ...
            <table class="data-table" id="reminders-table" style="width: 100%; border-collapse: collapse; text-align: left; font-size: 14px;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--border); color: var(--text-muted); font-weight: 600;">
                        ...
                        <th style="padding: 12px; width: 20%;">Lead Details</th>
...
<script>
    async function loadReminders() {
        try {
            ...
            if (data.error) { 
                tbody.innerHTML = `<tr><td colspan='6' style='text-align:center;color:red;padding:20px;'>Failed to load reminders: ${data.error}</td></tr>`; 
                return; 
            }
            if (!Array.isArray(data)) {
                tbody.innerHTML = `<tr><td colspan='6' style='text-align:center;color:red;padding:20px;'>API Error: Expected array, got ${typeof data}</td></tr>`;
                return;
            }
            if (data.length === 0) {
                tbody.innerHTML = `<tr><td colspan='6' style='text-align:center;padding:20px;color:var(--text-light);'>No pending reminders set.</td></tr>`;
                return;
            }
            ...
            data.forEach(r => {
                ...
                html += `<tr>
                    ...
                    <td>
                        <strong>ID: ${r.lead_id}</strong>
                        <div style="font-size:11px;color:var(--text-light);margin-top:2px;">Assigned to: @${r.assigned_to}</div>
                    </td>
                    <td style="color:var(--text-primary); font-weight:500;">
                        ${r.notes || '<span style="color:var(--text-light); font-style:italic;">No notes specified</span>'}
                    </td>
                    <td>
                        <span class="badge" style="background:${r.status==='Pending'?'#fee2e2':'#dcfce7'};color:${r.status==='Pending'?'#ef4444':'#166534'};padding:4px 8px;border-radius:4px;font-size:11px;font-weight:600;">
                            ${r.status}
                        </span>
                    </td>
                    <td style="text-align:right;">
                        ${r.status === 'Pending' ? `<button class="btn btn-primary" style="padding:4px 8px; font-size:12px; cursor:pointer;" onclick="completeReminder(${r.id})">Done</button>` : ''}
                    </td>
                </tr>`;
            });
            ...
            console.error(e);
            let tbody = document.querySelector("#reminders-table tbody");
            if (tbody) tbody.innerHTML = `<tr><td colspan='6' style='text-align:center;color:red;padding:20px;'>JS Exception: ${e.message}</td></tr>`;
        }
    }

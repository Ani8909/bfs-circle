...
$pl_today    = (int)$db->query("SELECT COUNT(*) FROM pre_leads WHERE date(created_at)='$today'")->fetchColumn();
...
$top_referrals   = $db->query("SELECT full_name as name, commission_rate FROM referrals WHERE status='Active' ORDER BY created_at DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
...
echo json_encode([
    'pre_leads'   => ['total'=>$pl_total,'new'=>$pl_new,'followup'=>$pl_followup,'junk'=>$pl_junk,'today'=>$pl_today],

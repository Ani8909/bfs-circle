...
$page_title = 'Part-Prepayment Calculator';
$page_subtitle = 'Analyze interest savings by making a lump-sum payment';
...
<div class="view-container">
    ...
    <div class="calc-layout">
        <div class="calc-inputs">
            ...
            <div class="input-group"><label>Outstanding Loan</label><div class="input-wrapper"><span class="prefix">₹</span><input type="number" id="amt" value="2000000" oninput="calc()"></div></div>
            <div class="input-group"><label>Current EMI</label><div class="input-wrapper"><span class="prefix">₹</span><input type="number" id="emi" value="25000" oninput="calc()"></div></div>
            <div class="input-group"><label>ROI (%)</label><div class="input-wrapper"><input type="number" id="roi" value="9" oninput="calc()"></div></div>
            <div class="input-group"><label>Lump-Sum Payment</label><div class="input-wrapper"><span class="prefix">₹</span><input type="number" id="lump" value="500000" oninput="calc()"></div></div>
        </div>
        <div class="calc-results">
            ...
            <div class="res-sub-grid">
                <div class="res-sub-item"><div class="s-lbl">Tenure Reduced By</div><div class="s-val" id="r-red">0</div></div><div class="res-sub-item"><div class="s-lbl">New Remaining Tenure</div><div class="s-val" id="r-new">0</div></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function calc() { 
            ...
            const e = parseFloat(document.getElementById('emi').value)||0;
            const rm = (parseFloat(document.getElementById('roi').value)||0)/1200;
            const lump = parseFloat(document.getElementById('lump').value)||0;
            if(rm<=0 || e<=p*rm) return;
            const oldN = -Math.log(1-(p*rm)/e)/Math.log(1+rm);
            const newP = p-lump;
            let newN=0; if(e>newP*rm) newN = -Math.log(1-(newP*rm)/e)/Math.log(1+rm);
            const oldInt = (e*oldN)-p; const newInt = (e*newN)-newP;
            document.getElementById('res-main').innerText = '₹'+fmt(oldInt-newInt);
            document.getElementById('r-red').innerText = Math.round(oldN-newN)+' Months';
            document.getElementById('r-new').innerText = Math.round(newN)+' Months';
            if(chart) { chart.data.datasets[0].data = [oldInt-newInt, newInt]; chart.update(); }
         }

...
// We rank by total approved leads (Phase 2, Phase 3, Phase 4, Completed)
$stmt = $db->query("
    SELECT 
...
foreach ($rankings as $index => $rank) {
    if ($rank['username'] === $agent_username) {
        $my_rank = $index + 1;
...
<div style="margin-bottom: 20px; text-align: center;">
    ...
    <h2 style="font-family: 'Outfit'; font-size: 22px; color: var(--text-primary);">Leaderboard</h2>
    <p style="color: var(--text-muted); font-size: 14px;">Top performers based on approved leads</p>
</div>

<!-- My Rank Card -->
<div class="card" style="background: linear-gradient(135deg, #064e3b, #065f46); color: white; display: flex; justify-content: space-between; align-items: center; padding: 20px;">
    <div>
        <div style="font-size: 13px; color: #94a3b8; font-weight: 500; margin-bottom: 4px;">My Current Rank</div>
        <div style="font-family: 'Outfit'; font-size: 28px; font-weight: 800; color: #f97316;">
            #<?php echo $my_rank; ?>
        </div>
    </div>
    <div style="text-align: right;">
        <div style="font-size: 20px; font-weight: 700;"><?php echo $my_approved; ?></div>
        <div style="font-size: 12px; color: #94a3b8;">Approved Leads</div>
    </div>
</div>
...
<div style="background: white; border-radius: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.02); overflow: hidden;">
    ...
    <?php if (count($rankings) > 0): ?>
        <?php foreach ($rankings as $index => $agent): 
            ...
            if ($rank_num == 1) { $rank_color = '#eab308'; $bg_color = 'rgba(234, 179, 8, 0.1)'; } // Gold
            if ($rank_num == 2) { $rank_color = '#94a3b8'; $bg_color = 'rgba(148, 163, 184, 0.15)'; } // Silver
            if ($rank_num == 3) { $rank_color = '#d97706'; $bg_color = 'rgba(217, 119, 6, 0.1)'; } // Bronze
            
            $is_me = ($agent['username'] === $agent_username);
        ?>
            <div style="display: flex; align-items: center; padding: 16px 20px; border-bottom: 1px solid var(--border); <?php echo $is_me ? 'background: rgba(249, 115, 22, 0.05);' : ''; ?>">
                ...
                <div style="flex: 1; margin-left: 12px;">
                    ...
                    <div style="font-size: 12px; color: var(--text-muted); margin-top: 2px;">
                        Total Leads: <?php echo $agent['total_leads']; ?>
                    </div>
                </div>
    ...
    <?php else: ?>
        <div style="text-align: center; padding: 40px 20px; color: var(--text-muted);">
            ...
            <p>No leads submitted yet to generate leaderboard.</p>
        </div>
    <?php endif; ?>
